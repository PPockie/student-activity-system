...
const NAV_ITEMS: NavItem[] = [
  { label: 'ภาพรวมระบบ', to: '/admin', icon: LayoutDashboard, end: true },
  { label: 'จัดการผู้ใช้', to: '/admin/users', icon: Users },
]

/** Layout ของกลุ่ม (admin) — โครงเดียวกับ teacher แต่คนละเมนู */
  ...
  return (
    <div className="bg-surface flex min-h-dvh">
      <div className="sticky top-0 z-20 hidden h-dvh md:block">
        <Sidebar items={NAV_ITEMS} subtitle={ROLE_LABEL.admin} footer={sidebarFooter} />
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="border-ink-100 sticky top-0 z-10 flex h-16 items-center gap-3 border-b bg-white/90 px-4 backdrop-blur md:px-8">
          ...
          <p className="flex-1 truncate font-semibold">แผงควบคุมผู้ดูแลระบบ</p>
